...
SkillPulse 根据真实执行记录持续评估每个 Agent Skill 版本的运行表现，通过统计方法识别显著退化，区分环境变化、模型切换、任务分布变化和 Skill 自身缺陷，并管理一套完整的 **检测 → 归因 → 提交候选版本 → 离线回放 → 灰度验证 → 晋升/回滚** 流程。候选版本的内容由人工、LLM 或外部规则程序生成，SkillPulse 负责存储、验证和状态管理，不负责自动编写修复内容。
...
- **验证外部候选版本**：人工、LLM 或规则程序生成的候选版本先接受历史样本回放；通过后才进入 probation 并承接少量线上调用。
...
# 提交由人工、LLM 或规则程序编写的候选版本；此时不能承接线上流量
...
if manager.scan():
    # 候选内容由外部修复提供方生成；SkillPulse 不负责生成内容
...
SkillPulse 不会自动编写新的 Skill 内容。`repair_fn` 是外部扩展点，可以接入：

- 人工修改后的 `SKILL.md` 或其他 Skill 文件；
- LLM 生成或修改后的完整候选内容；
- 针对明确错误的规则程序或代码转换器。

CLI 要求显式提供 `--content-file`，不会在缺少候选内容时生成占位版本。SkillPulse 的职责从 candidate 开始：保存版本、离线回放、线上灰度、晋升或拒绝。
...
4. 外部修复提供方生成候选内容，并以 candidate 状态提交，暂不承接线上流量。
